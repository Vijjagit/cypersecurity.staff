...
if (!isset($_GET['id']) || empty($_GET['id'])) {
    $error = 'ไม่พบรหัสบุคลากรที่ต้องการแก้ไข';
} else {
    $id = $_GET['id'];
    ...
    try {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (empty($firstName) || empty($lastName) || empty($email)) {
                ...
                if ($stmt->execute()) {
                    ...
                    $error = 'ไม่สามารถอัปเดตข้อมูลบุคลากรได้';
                }
            }
        }
        
        // ดึงข้อมูลบุคลากรที่ต้องการแก้ไข
        ...
        if (!$employee) {
            $error = 'ไม่พบข้อมูลบุคลากรที่ต้องการแก้ไข';
        }
        ...
        <?php if ($employee): ?>
            <form method="post">
                ...
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control" id="salary" name="salary" min="0" value="<?php echo $employee['salary']; ?>">
                </div>
                
                <button type="submit" class="btn btn-primary">บันทึกข้อมูล</button>
                <a href="read.php" class="btn btn-secondary">ยกเลิก</a>
            </form>
        <?php else: ?>
            <a href="read.php" class="btn btn-secondary">กลับไปหน้ารายการ</a>
        <?php endif; ?>
    </div>
</div>
